...
weatherApiKey ="e2949448cf5331bc"
...
class weatherPlugin(Plugin):
    localizations = {"weatherForecast": 
                        {"search":{
                            0:{"de-DE": u"Eventjes geduld aub...", "en-US": "Checking my sources..."},
                            1:{"de-DE": u"Eventjes geduld aub ...", "en-US": "Please wait while I check that..."},
                            2:{"de-DE": u"Eventjes geduld aub...", "en-US": "One moment please..."},
                            3:{"de-DE": u"Eventjes geduld aub... ...", "en-US": "Trying to get weather data for this location..."},
                            }, 
                        "forecast":{
                            "DAILY": {
                                0:{"de-DE": u"Hier is het weer voor {0}, {1}", "en-US": "Here is the forecast for {0}, {1}"},
                                1:{"de-DE": u"Hier is het weer voor {0}, {1}", "en-US": "This is the forecast for {0}, {1}"},
                                2:{"de-DE": u"Ik heb dit weerbericht voor je gevonden {0}, {1} gefunden", "en-US": "I found the following forecast for {0}, {1}"},
                                },
                            "HOURLY": {
                                0:{"de-DE": u"Hier het weer voor vandaag van {0}, {1}", "en-US": "Here is today's forecast for {0}, {1}"},
                                1:{"de-DE": u"Hier het weer voor vandaag van {0}, {1}", "en-US": "This is today's forecast for {0}, {1}"},
                                2:{"de-DE": u"Het weer vandaag {0}, {1} gefunden", "en-US": "I found the following hourly forecast for {0}, {1}"},
                                }
                            },
                        "failure": {
                                   "de-DE": "Sorry, ik kan het weer voor deze locatie niet vinden", "en-US": "I'm sorry but I could not find the forecast for this location!"
                                   }
                            }
                        }
        
    @register("de-DE", "(.*weer.*)|(.*weersverwachting.*)")     
	...
	speech = speech.replace(u".","")
        ...
        if (speech.count("vandaag") > 0 or speech.count("moment") > 0 or speech.count(u"nächsten Stunden") > 0 or speech.count(u"für heute") > 0) and language=="de-DE":
            viewType = "HOURLY"
            speech = speech.replace("vandaag","")
            speech = speech.replace("vandaag","")
            speech = speech.replace("nu","")
            speech = speech.replace("actuele","")
            speech = speech.replace(u"Voor vandaag"," in ")
            speech = speech.replace(u"Voor"," in ")
        
        if language=="en-US":
            speech = speech.replace(" for "," in ")
            
        if language=="de-DE":
            speech = speech.replace(u"de volgende dag","")
            speech = speech.replace(u"in de volgende paar dagen","")
            speech = speech.replace(u" voor "," in ")
