...
class HandleConnection(ssl_dispatcher):
    ...
    def __init__(self, conn):
        ...
        self.ssled = False
        self.secure_connection(certfile="certs/server.passless.crt", keyfile="certs/server.passless.key", server_side=True)               
...
caCertFile = open('certs/OrigAppleSubCACert.der')
caCert = X509.load_cert_bio(BIO.MemoryBuffer(caCertFile.read()), format=0)
caCertFile.close()
certFile = open('certs/OrigAppleServerCert.der')
